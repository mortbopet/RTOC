...
#define FG_CHECK(func, fg)\
    if(func != FG_OK){\
    std::stringstream ss;\
    ss <<"Error (" <<Fg_getLastErrorNumber(fg) << ") " \
    <<  Fg_getLastErrorDescription(fg) << std::endl; \
...
void FgExample::drawBuffer(int32_t bufferIndex){
    mDisplay->drawBuffer(bufferIndex);
}
...
uint32_t FgExample::getDmaNumber(Fg_Struct* fgHandle){
    ...
    int32_t status = Fg_getParameterProperty(fgHandle, 
        FG_NR_OF_DMAS, 
        PROP_ID_VALUE, 
        dataCharArr.get(), 
        &dataSize);
